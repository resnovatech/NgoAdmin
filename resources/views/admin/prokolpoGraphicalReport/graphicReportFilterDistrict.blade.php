...
@else
<p>বছর: {{  App\Http\Controllers\Admin\CommonController::englishToBangla($prokolpoYear.' - '.$prokolpoYear+1)  }}</p>
@endif
...
@if(empty($prokolpoTypeId))
প্রকল্পের ধরণ: সকল প্রকল্প
@else
<p>প্রকল্পের ধরণ:

    @foreach($prokolpoTypeId as $prokolpoTypeIds)
    {{ DB::table('project_subjects')->where('id',$prokolpoTypeIds)->value('name')}}
    @endforeach
</p>
@endif
