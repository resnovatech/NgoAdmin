...
@foreach ($senderNothiListformNoFive->unique('receiver') as $key=>$nothiLists1)
...
<?php
$getLastId = DB::table('parent_note_for_form_no_five_daks')->where('nothi_detail_id',$nothiLists1->dakId)
...
$getLastIdDate = DB::table('child_note_for_form_no_fives')->where('pnote_form_no_five',$getLastId)
//->where('serial_number',$nothiLists1->nothId)
->orderBy('id','desc')
->first();
...
data-original-title="" data-bs-target="#myModalformNoFive{{ $key+1 }}">
নথি গতিবিধি
</button>
@endif
...
@include('admin.documentMovementModal.formNoFiveModal')
...
if($nothiLists1->dakType == 'formNoFive'){
...
$allNoteListNew = DB::table('parent_note_for_form_no_five_daks')->where('nothi_detail_id',$nothiLists1->dakId)
->where('serial_number',$nothiLists1->nothId)
