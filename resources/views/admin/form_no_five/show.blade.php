...
<div class="container-fluid">
    <div class="page-header">
        <div class="row">
            <div class="col-sm-6">
                <h3>বার্ষিক প্রতিবেদন ফরম </h3>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">হোম</a></li>
                    <li class="breadcrumb-item">ফরম নং - ৫</li>
                    <li class="breadcrumb-item">ফরম নং - ৫ এর বিবরণ</li>
                </ol>
            </div>
            <div class="col-sm-6">
            </div>
            ...
            <div class="card-body">
                ...
                <ul class="nav nav-dark" id="pills-darktab" role="tablist">
                    <li class="nav-item"><a class="nav-link active" id="pills-darkhome-tab"
                                    ...
                                    class="icofont icofont-ui-home"></i>ফরম নং - ৫</a></li>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <form id="form" action="{{ route('statusUpdateForformNoFive') }}" method="post">
                                                @csrf


                                                <input type="hidden" value="{{ $dataFromNoFiveForm->mainId }}" name="id" />
                                                ...
                                                <select class="form-control form-control-sm mt-4" name="status" id="regStatus">
                                                    ...
                                                    <option value="pending" {{ $dataFromNoFiveForm->status == 'pending' ? 'selected':''  }}>চলমান</option>

                                                    <option value="Accepted" {{ $dataFromNoFiveForm->status == 'Accepted' ? 'selected':''  }}>গৃহীত</option>
                                                    <option value="Correct" {{ $dataFromNoFiveForm->status == 'Correct' ? 'selected':''  }}>সংশোধন করুন</option>
                                                    <option value="Rejected" {{ $dataFromNoFiveForm->status == 'Rejected' ? 'selected':''  }}>প্রত্যাখ্যান করুন</option>

                                                </select>


                                                @if($dataFromNoFiveForm->status == 'Correct' || $dataFromNoFiveForm->status == 'Rejected')

                                                <div id="rValueStatus" >
                                                    <label>বিস্তারিত লিখুন:</label>
                                                    <textarea class="form-control form-control-sm" name="comment">{{ $dataFromNoFiveForm->comment }}</textarea>
                                                </div>
                                                @else
