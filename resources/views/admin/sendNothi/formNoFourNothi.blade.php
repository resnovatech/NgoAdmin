...
@foreach ($senderNothiListformNoFour->unique('receiver') as $key=>$nothiLists1)
...
<?php
$getLastId = DB::table('parent_note_for_form_no_fours')
...
$getLastIdDate = DB::table('child_note_for_form_no_fours')
->where('pnote_form_no_four',$getLastId)
//->where('serial_number',$nothiLists1->nothId)
->orderBy('id','desc')
->first();
...
data-original-title="" data-bs-target="#myModalformNoFour{{ $key+1 }}">
নথি গতিবিধি
</button>
@endif
...
@include('admin.documentMovementModal.formNoFourModal')
...
if($nothiLists1->dakType == 'formNoFour'){
...
$allNoteListNew = DB::table('parent_note_for_form_no_fours')->where('nothi_detail_id',$nothiLists1->dakId)
->where('serial_number',$nothiLists1->nothId)
