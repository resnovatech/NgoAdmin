...
@foreach ($senderNothiListformNoSeven->unique('receiver') as $key=>$nothiLists1)
...
<?php
$getLastId = DB::table('parent_note_for_form_no_seven_daks')
...
$getLastIdDate = DB::table('child_note_for_form_no_sevens')
->where('pnote_form_no_seven',$getLastId)
//->where('serial_number',$nothiLists1->nothId)
->orderBy('id','desc')
->first();
...
@include('admin.documentMovementModal.formNoSevenModal')
...
if($nothiLists1->dakType == 'formNoSeven'){
...
$allNoteListNew = DB::table('parent_note_for_form_no_sevens')->where('nothi_detail_id',$nothiLists1->dakId)
->where('serial_number',$nothiLists1->nothId)
