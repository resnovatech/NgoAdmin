...
//end new code
    ...
    ->where('id',$decesionNameId)->where('status','fdFourOneForm')->value('decision_list');

                    ?>

                     @if(!empty($checkDataAvailableOrNot))

                     @include('admin.post.receivedfdFourOneFormDak')
...
<!-- red background end -->
                    ...
                    <td style="text-align:right;">
                        <button class="btn btn-primary btn-xs" type="button" data-original-title="btn btn-danger btn-xs" title="" onclick="location.href = '{{ route('showDataAll',['status'=>'fdFourOneForm','id'=>$allStatusData->id]) }}';">প্রেরণ</button>
                     ...
                     data-original-title="" data-bs-target="#myModalfdFourOneFormForm{{ $ps }}">
                     ডাক গতিবিধি
             </button>
             ...
             id="myModalfdFourOneFormForm{{ $ps }}" tabindex="-1" role="dialog"
