...
//end new code
    ...
    ->where('id',$decesionNameId)->where('status','formNoFour')->value('decision_list');

                    ?>

                     @if(!empty($checkDataAvailableOrNot))

                     @include('admin.post.receivedFormNoFourDak')
...
<!-- red background end -->
                    ...
                    <td style="text-align:right;">
                        <button class="btn btn-primary btn-xs" type="button" data-original-title="btn btn-danger btn-xs" title="" onclick="location.href = '{{ route('showDataAll',['status'=>'formNoFour','id'=>$allStatusData->id]) }}';">প্রেরণ</button>
                     ...
                     data-original-title="" data-bs-target="#myModalformNoFourForm{{ $ps }}">
                     ডাক গতিবিধি
             </button>
             ...
             id="myModalformNoFourForm{{ $ps }}" tabindex="-1" role="dialog"
