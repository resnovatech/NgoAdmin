<?php
    $ngoStatusFdFourOne =DB::table('fd4_one_form_daks')->where('status',1)
...
@foreach($ngoStatusFdFourOne as $i=>$allStatusData)
<?php
...
$formOneDataId = DB::table('ngo_statuses')->where('id',$allStatusData->fd4_one_form_status_id)
...
->where('id',$allStatusData->dak_detail_id)->where('status','fdFourOneForm')->value('decision_list');
?>
...
<td style="text-align:right;">
    ...
    data-original-title="" data-bs-target="#fdthreemyModalfdFourOneForm{{ $allStatusData->id }}">
<i class="fa fa-reply"></i> নথিতে উপস্থাপন করুন
</button>

@include('admin.post.fdFourOneFormNothiModal')
...
data-bs-toggle="modal"
data-original-title="" data-bs-target="#myModalregfdFourOneForm{{ $i }}">
ডাক গতিবিধি
</button>
...
id="myModalregfdFourOneForm{{ $i }}" tabindex="-1" role="dialog"
