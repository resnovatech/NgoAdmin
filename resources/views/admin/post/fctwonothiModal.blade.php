...
<div class="modal right fade bd-example-modal-lg"
     id="fctwomyModal{{ $allStatusData->id }}" tabindex="-1" role="dialog"
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12 mt-3">
                        <div class="input-group">
                            <input class="form-control"  id="nothiSearchFctw{{ $allStatusData->id }}" type="text" placeholder="নথি খুজুন"><span
                    ...
                    <div class="col-lg-12 mt-3">
                        <div class="table-responsive">
                            <div class="accordion accordion-flush" id="accordionFlushExample">
                                <table class="table table-striped" id="nothiSearchResultFctw{{ $allStatusData->id }}">
                                    <tbody>
                                                ...
                                                <div id="flush-collapse{{ $key+1 }}"
                                                    ...
                                                    <div class="accordion-body">
                                                        <div class="d-flex mt-3">
                                                            <button onclick="location.href = '{{ route('addParentNote',['status'=>'fcTwo','dakId'=>$allStatusData->id,'nothiId'=>$nothiLists->id]) }}';" class="btn btn-transparent ms-3" type="button">
  ...
  ->where('nothi_detail_id',$allStatusData->id)->get();
    ...
    <ul>
        @foreach($allNoteListNew as $key=>$allNoteListNews)
                                                          <li>  {{ App\Http\Controllers\Admin\CommonController::englishToBangla(($key+1)) }} .   <a href="{{ route('addChildNote', ['status' =>'fcTwo','parentId'=>$allStatusData->id,'nothiId'=>$nothiLists->id,'id' =>$allNoteListNews->id,'activeCode' => ($key+1)]) }}">{{ $allNoteListNews->subject }}</a> </li>
                                                                @endforeach
                                                            </ul>
