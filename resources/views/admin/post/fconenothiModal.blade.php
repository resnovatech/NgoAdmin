    ...
    <div class="modal-dialog modal-lg-custom"
		...
		<button id="pp" class="btn btn-outline-danger btn-sm"><i class="fa fa-times" aria-hidden="true"></i></button>
            ...
            <div class="modal-body">
                <h5>একশনঃ নথিতে উপস্থাপনঃ এফসি-১  </h5>
                <div class="row">
                    <div class="col-lg-6">
                        <h5>সকল নথি</h5>
                    </div>
                    ...
                    <div class="col-lg-12 mt-3">
                        <div class="input-group">
                            <input class="form-control"  id="nothiSearchFcon{{ $allStatusData->id }}" type="text" placeholder="নথি খুজুন"><span
                    ...
                    <div class="col-lg-12 mt-3">
                        <div class="table-responsive">
                            <div class="accordion accordion-flush" id="accordionFlushExample">
                                <table class="table table-striped" id="nothiSearchResultFcon{{ $allStatusData->id }}">
                                    <tbody>
                                                ...
                                                <div id="flush-collapse{{ $key+1 }}"
                                                    ...
                                                    <div class="accordion-body">
                                                        <div class="d-flex mt-3">
                                                            <button onclick="location.href = '{{ route('addParentNote',['status'=>'fcOne','dakId'=>$allStatusData->id,'nothiId'=>$nothiLists->id]) }}';" class="btn btn-transparent ms-3" type="button">
  ...
  ->where('nothi_detail_id',$allStatusData->id)->get();
    ...
    <ul>
        @foreach($allNoteListNew as $key=>$allNoteListNews)
                                                          <li>  {{ App\Http\Controllers\Admin\CommonController::englishToBangla(($key+1)) }} .   <a href="{{ route('addChildNote', ['status' =>'fcOne','parentId'=>$allStatusData->id,'nothiId'=>$nothiLists->id,'id' =>$allNoteListNews->id,'activeCode' => ($key+1)]) }}">{{ $allNoteListNews->subject }}</a> </li>
                                                                @endforeach
                                                            </ul>
